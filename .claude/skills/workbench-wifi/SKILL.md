...
```bash
# Check current mode
curl http://pi4b.local:8080/api/wifi/mode

# Switch to wifi-testing mode
curl -X POST http://pi4b.local:8080/api/wifi/mode \
...
curl http://pi4b.local:8080/api/wifi/ap_status

# Stop AP
curl -X POST http://pi4b.local:8080/api/wifi/ap_stop
```
...
```bash
# Join a network
curl -X POST http://pi4b.local:8080/api/wifi/sta_join \
  -H 'Content-Type: application/json' \
  -d '{"ssid": "MyNetwork", "pass": "password", "timeout": 15}'

# Disconnect
curl -X POST http://pi4b.local:8080/api/wifi/sta_leave
```

## WiFi Scan

```bash
curl http://pi4b.local:8080/api/wifi/scan
```
...
curl "http://pi4b.local:8080/api/wifi/events?timeout=30"
```
